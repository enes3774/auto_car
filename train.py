...
class HLGaussLoss(nn.Module):
    ...
    Creates soft targets using Gaussian distribution centered at true bucket.
    Penalizes predictions less when they're closer to the true bucket.
    """
    
    def __init__(self, num_buckets: int = 64, sigma: float = 0.75):
        """
        Args:
            num_buckets: Number of value buckets (64)
            sigma: Standard deviation of Gaussian smoothing
                   - Lower sigma = sharper distribution (more like one-hot)
                   - Higher sigma = smoother distribution (more tolerance)
        """
        super().__init__()
        self.num_buckets = num_buckets
        self.sigma = sigma
        
        # Pre-compute bucket indices for efficiency
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            logits: Predicted logits (batch_size, num_buckets)
            targets: Target bucket indices (batch_size,)
        ...
        # Ensure bucket_indices is on correct device
        bucket_indices = self.bucket_indices.to(device)
        
        # Expand targets for broadcasting: (batch_size, 1)
        targets_expanded = targets.unsqueeze(1).float()
        
        # Compute Gaussian soft targets: (batch_size, num_buckets)
        # exp(-0.5 * ((i - target) / sigma)^2)
        distances = bucket_indices.unsqueeze(0) - targets_expanded
        soft_targets = torch.exp(-0.5 * (distances / self.sigma) ** 2)
        
        # Normalize to sum to 1
        soft_targets = soft_targets / soft_targets.sum(dim=1, keepdim=True)
        
        # KL divergence: -sum(soft_targets * log_softmax(logits))
...
class ChessMAVLoss(nn.Module):
    ...
    def __init__(
        ...
        self.hl_gauss = HLGaussLoss(num_buckets=num_buckets, sigma=sigma)
